...
public class ADCMovementSystemBackup: System {
    ...
    static let query = EntityQuery(where: .has(ADCComponent.self) && .has(ADCComponent.self, where: { $0.state == .moving }))
    ...
    public func update(context: SceneUpdateContext) {
        ...
        // print("\n=== ADC Movement System Update ===")
        // print("Found \(entities) entities with ADCComponent")
        ...
        for entity in entities {
            guard var adcComponent = entity.components[ADCComponent.self],
                ...
                print("⚠️ Entity \(entity.name) failed guard check:")
                print("- Has ADCComponent: \(entity.components[ADCComponent.self] != nil)")
                print("- State is moving: \(entity.components[ADCComponent.self]?.state == .moving)")
                print("- Has start position: \(entity.components[ADCComponent.self]?.startWorldPosition != nil)")
                print("- Has target ID: \(entity.components[ADCComponent.self]?.targetEntityID != nil)")
                continue 
            }
...
//                    print("Target Orientation: \(simd_quatf(from: SIMD3<Float>(0, 0, 1), to: tangent))")
    ...
    private static func updateProteinSpin(entity: Entity, deltaTime: TimeInterval) {
        if let proteinComplex = entity.findEntity(named: "antibodyProtein_complex_assembled") {
    ...
    public static func startMovement(entity: Entity, from start: SIMD3<Float>, to targetPoint: Entity) {
       ...
       print("ADC Component State: \(entity.components[ADCComponent.self]?.state)")
        ...
        entity.components[ADCComponent.self] = adcComponent
        print("Final component state: \(entity.components[ADCComponent.self]?.state)")
